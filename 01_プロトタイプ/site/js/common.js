...
function appoint_click(num) {
   ...
   let id = elements.item(0).rows[num].cells[0].firstChild.data;
   let id2 =
      elements.item(0).rows[num].cells[1].firstChild.data +
      "　" +
      elements.item(0).rows[num].cells[2].firstChild.data;
...
function button_kanryou4() {
   ...
   if (let1 == true) {
      ...
      if (count1 % 2 == 0) {
         ...
         target2.innerHTML =
            "<font color='red'>※登録処理に失敗しました。<br>もう一度登録し直すかサポートに連絡してください。</font>";
      }
   }
}
...
function error_button() {
   ...
   if (let3 == true) {
      ...
      if (count % 2 == 0) {
         ...
         target1.innerHTML =
            "※入力エラーがあります。メッセージを確認して再入力してください";
      }
   }
   if (let4 == true) {
